...
public class FollowController {
        ...
        public ResponseEntity<ApiResponse<Page<FriendResponse>>> getFollowers(
                        @PathVariable Long userId,
                        @RequestParam(defaultValue = "0") int page,
                        @RequestParam(defaultValue = "20") int size) {
                Page<FriendResponse> data = followService.getFollowers(userId, page, size);
        ...
        public ResponseEntity<ApiResponse<Page<FriendResponse>>> getFollowing(
                        @PathVariable Long userId,
                        @RequestParam(defaultValue = "0") int page,
                        @RequestParam(defaultValue = "20") int size) {
                Page<FriendResponse> data = followService.getFollowing(userId, page, size);
                return ResponseEntity.ok(ApiResponse.<Page<FriendResponse>>builder()
                                .status(ErrorCode.SUCCESS.getStatus())
                                .message(ErrorCode.SUCCESS.getMessage())
                                .data(data)
